...
// @PSMod
// Ok so we want to intercept this and then make it the 1st of the next month
                        $LBFOrderMonth = date("m",strtotime($from_date));
                        $LBFOrderYear = date("y",strtotime($from_date));
                        $LBF_nextMonthBegin = mktime(0,0,1,date($LBFOrderMonth)+1,+1,date($LBFOrderYear)); 
                        $LBF_nextMonthBegin = date( 'Y-m-d H:i:s', $LBF_nextMonthBegin );
                        $next_payment_timestamp = strtotime( $LBF_nextMonthBegin );
