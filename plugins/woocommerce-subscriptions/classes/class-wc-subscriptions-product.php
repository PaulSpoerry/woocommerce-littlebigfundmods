...
// @PSMod
		$subscription_length = self::get_length( $product_id );
                ...
                // Business Decision: Any 1 month subscription length will NOT have a trial period
                // $trial_period        = self::get_trial_period( $product_id );
                // $trial_length        = self::get_trial_length( $product_id );
		if ( $subscription_length == 1 && $subscription_period == 'month' ){
                        if( empty( $order_date ) )
                                    $order_date = date( 'Y-m-d H:i:s' );
                        // http://sandbox.onlinephpfunctions.com/code/fromFunction/strtotime
                        $order_date = date( 'Y-m-d H:i:s' );
                        $LBFOrderMonth = date("m",strtotime($order_date));
                        $LBFOrderDay = date("d",strtotime($order_date));
                        $LBFOrderYear = date("y",strtotime($order_date));
                        $LBF_thisMonthEnd = mktime(23,59,59,date($LBFOrderMonth)+1,+0,date($LBFOrderYear)); 
                        $LBF_nextMonthEnd = mktime(23,59,59,date($LBFOrderMonth)+2,-0,date($LBFOrderYear)); 
                        $LBF_nextMonthBegin = mktime(0,0,1,date($LBFOrderMonth)+1,+1,date($LBFOrderYear)); 
                        /* echo "order date: " . $order_date;
                        echo  "\nOrder Month: " . $LBFOrderMonth;
                        echo  "\nOrder Day: " . $LBFOrderDay;
                        echo  "\nOrder Year: " . $LBFOrderYear;
                        echo  "\nThis month end: " . date( 'Y-m-d H:i:s', $LBF_thisMonthEnd);
                        echo  "\nNext month end: " . date( 'Y-m-d H:i:s', $LBF_nextMonthEnd );
                        echo  "\nNext month begin: " . date( 'Y-m-d H:i:s', $LBF_nextMonthBegin ); 
                        echo "\nreturns" . $expiration_date */
                        //expiration for one month subscriptions will ALWAYS be the beginning of the next month
                        $expiration_date = date( 'Y-m-d H:i:s', $LBF_thisMonthEnd);
                } elseif ( $subscription_length > 0 ){
